...
class DRIT(nn.Module):
  ...
  def get_contrastive_loss(self, content_a, content_b, labels_a, labels_b, margin):
    ...
    dist_sim = torch.norm(content_a - content_b, dim=1)/1000
  ...
  def backward_EG(self):
    ...
    loss_cross_content_A = self.criterionL1(self.z_content_a, self.z_content_recon_a)
    loss_cross_content_B = self.criterionL1(self.z_content_b, self.z_content_recon_b)

    loss_contrastive = self.get_contrastive_loss(self.z_content_a, self.z_content_b, self.label_A, self.label_B, margin=10)
    ...
    loss_G = loss_G_L1_AA * 10 + loss_G_L1_BB * 10 + \
             loss_G_L1_A * 10 + loss_G_L1_B * 10 + \
  ...
  def assemble_outputs(self):
    ...
    a = images_b1[0:1, ::].view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()
    # b = images_b2.view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()
    c = images_a4[0:1, ::].view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()
    d = images_a3[0:1, ::].view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()

    e = images_a1[0:1, ::].view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()
    # f = images_a2.view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()
    g = images_b4[0:1, ::].view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()
    h = images_b3[0:1, ::].view(images_a1.shape[1], images_a1.shape[2], images_a1.shape[3]).cpu().numpy()
