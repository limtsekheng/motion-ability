...
class DRIT(nn.Module):
  ...
  def get_contrastive_loss(self, content_a, content_b, labels_a, labels_b, margin):
    lst = list(zip(labels_a, labels_b))
    res = [1 if item[0]==item[1] else 0 for item in lst]
    labels = torch.FloatTensor(res).cuda(self.gpu)

    content_a = content_a.view(content_a.shape[0], -1)
    content_b = content_b.view(content_b.shape[0], -1)

    dist_sim = torch.norm(content_a - content_b, dim=1) * 1
    dist_diss = torch.clamp(margin - dist_sim, min = 0)

    loss = labels * torch.pow(dist_sim, 2) + (1 - labels) * torch.pow(dist_diss, 2)
    loss = 0.5 * torch.sum(loss) / content_a.shape[0]
    # print('dist_sim', dist_sim)
    # print('dist_diss', dist_diss)
    # print('loss', loss)
    return loss



  def input_for_forward(self, images_a, labels_a, images_b, labels_b):
    self.input_A = images_a
    self.input_B = images_b
    self.label_A = labels_a
    self.label_B = labels_b


  def forward(self):
    # input images
    self.real_A_encoded = self.input_A
    self.real_B_encoded = self.input_B  
    # half_size = 1
    # real_A = self.input_A
    # real_B = self.input_B
  ...
  def update_D(self, image_a, image_b):
    ...
    nn.utils.clip_grad_norm_(self.disA.parameters(), 1)
    self.disA_opt.step()
    ...
    # nn.utils.clip_grad_norm_(self.disA2.parameters(), 1)
    # self.disA2_opt.step()
    ...
    nn.utils.clip_grad_norm_(self.disB.parameters(), 1)
    self.disB_opt.step()
    ...
    # nn.utils.clip_grad_norm_(self.disB2.parameters(), 1)
    # self.disB2_opt.step()
  ...
  def update_EG(self):
    ...
    nn.utils.clip_grad_norm_(self.enc_c.parameters(), 1)
    nn.utils.clip_grad_norm_(self.enc_a.parameters(), 1)
    nn.utils.clip_grad_norm_(self.gen.parameters(), 1)
    self.enc_c_opt.step()
    self.enc_a_opt.step()
    self.gen_opt.step()
  ...
  def backward_EG(self):
    ...
    loss_G_L1_A = self.criterionL1(self.fake_A_recon, self.real_A_encoded) * 50
    loss_G_L1_B = self.criterionL1(self.fake_B_recon, self.real_B_encoded) * 50
    loss_G_L1_AA = self.criterionL1(self.fake_AA_encoded, self.real_A_encoded) * 50
    loss_G_L1_BB = self.criterionL1(self.fake_BB_encoded, self.real_B_encoded) * 50

    loss_cross_content_A = self.criterionL1(self.z_content_a, self.z_content_recon_a) * 100
    loss_cross_content_B = self.criterionL1(self.z_content_b, self.z_content_recon_b) * 100

    loss_contrastive = self.get_contrastive_loss(self.z_content_a, self.z_content_b, self.label_A, self.label_B, margin=2) * 5
    ...
    # loss_G = loss_G_L1_AA + loss_G_L1_BB + loss_G_L1_A + loss_G_L1_B

    loss_G = loss_G_L1_AA  + loss_G_L1_BB + \
             loss_G_L1_A  + loss_G_L1_B + \
             loss_contrastive + loss_cross_content_A + loss_cross_content_B
    ...
    self.contrastive_loss = loss_contrastive.item()
    self.cross_content_A_loss = loss_cross_content_A.item()
    self.cross_content_B_loss = loss_cross_content_B.item()
  ...
  def backward_G_alone(self):
    loss_GAN_A = self.backward_G_GAN(self.fake_A_encoded, self.disA)
    loss_GAN_B = self.backward_G_GAN(self.fake_B_encoded, self.disB)
    loss_G_domain = loss_GAN_A + loss_GAN_B
    loss_G_domain.backward()

    self.gan_a_loss = loss_GAN_A
    self.gan_b_loss = loss_GAN_B
  ...
  def assemble_outputs(self):
    ...
    # images_a2 = self.fake_A_random.detach()
    images_a3 = self.fake_A_recon.detach()
    images_a4 = self.fake_AA_encoded.detach()
    images_b1 = self.fake_B_encoded.detach()
    # images_b2 = self.fake_B_random.detach()
    images_b3 = self.fake_B_recon.detach()
    images_b4 = self.fake_BB_encoded.detach()

    a = images_b1[0:1, ::].view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])
    # b = images_b2.view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])
    c = images_a4[0:1, ::].view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])
    d = images_a3[0:1, ::].view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])

    e = images_a1[0:1, ::].view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])
    # f = images_a2.view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])
    g = images_b4[0:1, ::].view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])
    h = images_b3[0:1, ::].view(images_a1.shape[1], images_a1.shape[2]).cpu().numpy().reshape([3,72,200])
