...
class DRIT(nn.Module):
  def __init__(self, opts):
    ...
    self.disA_opt = torch.optim.RMSprop(self.disA.parameters(), lr=lr, weight_decay=0.0001)
    self.disB_opt = torch.optim.RMSprop(self.disB.parameters(), lr=lr, weight_decay=0.0001)
    self.disA2_opt = torch.optim.RMSprop(self.disA2.parameters(), lr=lr, weight_decay=0.0001)
    self.disB2_opt = torch.optim.RMSprop(self.disB2.parameters(), lr=lr, weight_decay=0.0001)
    self.disContent_opt = torch.optim.RMSprop(self.disContent.parameters(), lr=lr, weight_decay=0.0001)
    self.enc_c_opt = torch.optim.RMSprop(self.enc_c.parameters(), lr=lr, weight_decay=0.0001)
    self.enc_a_opt = torch.optim.RMSprop(self.enc_a.parameters(), lr=lr, weight_decay=0.0001)
    self.gen_opt = torch.optim.RMSprop(self.gen.parameters(), lr=lr, weight_decay=0.0001)
  ...
  def backward_D(self, netD, real, fake):
    ...
    for it, (out_a, out_b) in enumerate(zip(pred_fake, pred_real)):
      out_fake = out_a
      out_real = out_b
      loss_D += -(torch.mean(out_real) - torch.mean(out_fake))
    loss_D.backward()
    return loss_D
  ...
  def backward_EG(self):
    ...
    loss_G = loss_G_GAN_A + loss_G_GAN_B  + \
             loss_G_GAN_Acontent + loss_G_GAN_Bcontent + \
             loss_G_L1_AA * 50 + loss_G_L1_BB * 50 + \
             loss_G_L1_A * 50 + loss_G_L1_B * 50 #+ \
             # loss_kl_zc_a + loss_kl_zc_b + \
             # loss_kl_za_a + loss_kl_za_b              
