...
class Dis_domain(nn.Module):
  ...
  def __init__(self):
    ...
    model += [nn.Conv1d(in_channels = 1024, out_channels = 1, kernel_size = 3, stride = 1, padding = 1)]  # Nx1x13
    self.model = nn.Sequential(*model)
    self.fc = nn.Sequential(nn.Linear(13, 10))
...
class G(nn.Module):
  ...
  def forward_a(self, z_c, z_a):
    ...
    z_c_1 = z_c.view(z_c.size(0), z_c.size(1), 1).expand(z_c.size(0), z_c.size(1), out1.size(2))
    latent1 = torch.cat([out1, z_c_1],1)
    out2 = self.dec1_a(latent1)    # Nx504x25
    z_c_2 = z_c.view(z_c.size(0), z_c.size(1), 1).expand(z_c.size(0), z_c.size(1), out2.size(2))
    latent2 = torch.cat([out2, z_c_2],1)  
    out3 = self.dec2_a(latent2)    # Nx248x50
    z_c_3 = z_c.view(z_c.size(0), z_c.size(1), 1).expand(z_c.size(0), z_c.size(1), out3.size(2))
    latent3 = torch.cat([out3, z_c_3],1)
    out4 = self.dec3_a(latent3)    # Nx208x100
    z_c_4 = z_c.view(z_c.size(0), z_c.size(1), 1).expand(z_c.size(0), z_c.size(1), out4.size(2))
    latent4 = torch.cat([out4, z_c_4],1)  
